<template>
  <div class="tags-container">
    <div class="tag_left" v-if="isShow">
      <el-button icon="el-icon-arrow-left" size="mini" title="前一个" @click="moveForward"></el-button>
    </div>
    ...
    <div class="tag_right" v-if="isShow">
      <el-button icon="el-icon-arrow-right" size="mini" title="后一个" @click="backwardShift"></el-button>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  computed: {
    ...
    isFirstTag: function() {
      let nowIndex = this.tagList.findIndex(
        item => item.value === this.nowTagValue
      );
      return nowIndex === 0;
    },
    isLastTag: function() {
  ...
  methods: {
    ...
    goNext() {
      ...
      let nextIndex =
        nowIndex < this.tagList.length - 1
          ? nowIndex + 1
