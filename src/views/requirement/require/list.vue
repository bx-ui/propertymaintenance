      ...
      <el-row :gutter="24">
        ...
        <el-col :span="6">
          <el-form-item label="单据类型" label-width="80px">
            <el-select v-model="listQuery.billType" size="small" clearable filterable :disabled="this.listQuery.demandType == ''">
              <el-option
         ...
         <el-col :span="6">
          <el-form-item label="审批状态" label-width="80px">
            <el-select v-model="listQuery.type" size="small" clearable filterable>
              <el-option
                v-for="(item,index) in seeOption"
                :key="index"
                :label="item.name"
                :value="item.code"
              ></el-option>
...
import { dictTypeTree, deleteApplication } from "@/api/maintenance/dictionary";
export default {
  data() {
    return {
      timer: [],
      ...
      requireTypeOption: [],
      typeOption: [],
      seeOption: [{code: 1, name: "审批中"}, {code: 2, name: "审批完成"}, {code: 4, name: "审批驳回"}],
      multion: [],
      selId: []
    };
  },
  ...
  methods: {
    ...
    changeToType() {
      ...
      this.typeOption = [];
      console.log(this.listQuery.demandType);
    ...
    deleteList() {
      // 作废
      console.warn(this.selId);
      if (this.selId.length <= 0) {
        this.$message.warning("至少选择一条数据");
        return false;
      }
      var data = this.multion.map(v => v.state);
      if (data.indexOf("4") == -1) {
        this.$message.warning("只有审批驳回的数据才能作废");
        return false;
      }
      ...
      deleteApplication({ ids: this.selId }).then(res => {
        this.$message({
          type: "success",
          message: "作废成功"
        });
