...
<template>
  <div class="wrap">
    ...
    <div class="pane_bottom">
      ...
      >
        <el-table-column prop="materialName" label="物料名称" min-width="230" align="center">
        ...
        <el-table-column prop="count" label="用量" min-width="100" align="center">
          <template slot-scope="scope">
            <el-input
        ...
        <el-table-column prop="totalPrice" label="总额" align="center" min-width="120"></el-table-column>
        <el-table-column fixed="right" label="操作" align="center">
          <template slot-scope="scope">
            <el-button @click="handleDelete(scope.row.itemId)" type="text" size="mini">删除</el-button>
          </template>
        </el-table-column>
        <el-table-column prop="remark" label="备注" min-width="230" align="center">
          <template slot-scope="scope">
...
import { saveData } from "@/api/require/require";
...
export default {
  ...
  methods: {
    ...
    handleInputCount(value, id) {
      ...
      for (let i = 0; i < this.tableData.length; i++) {
        if (id == this.tableData[i].itemId) {
          // console.error(value,this.tableData[i].price)
    ...
    handleSubmit() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          obj.append(
            "projectName",
            this.projectOption.filter(
              item => item.projectId == this.form.projectUuid
            )[0].projectName
          );
          if (!!this.form.projectStageUuid) {
            obj.append(
              "projectStageName",
              this.stagetOption.filter(
                item => item.projectStageUuid == this.form.projectStageUuid
              )[0].projectStageName
            );
          } else {
            obj.append("projectStageName", "");
          }

          if (this.moneyListIsShow) {
            //物料
            obj.append(
              "hzPropertyApplicationLists",
