<template>
    <div class="app-container calendar-list-container">
        ...
        <div class="message-btns">
            <el-button size="mini" type="primary" @click="intGo">入职确认</el-button>
        </div>
        <!-- 列表 -->
        <msTable :tableList="tableList" :tableData="tableData" v-model="page" @refresh="refresh" @multData="multData" :height="height" :flagShowBtns="1" :flag="0"></msTable>
        </div>
</template>
...
export default {
        ...
        getList(){
            getpage(this.listQuery).then(res => {
                ...
                this.tableData = data.filter(item => {
                    return (item.state == '入职申请中' || item.state == '待上岗确认' || item.state == '入职驳回');
                });
    ...
    created(){
        this.getList();
        this.stateOption = [{code: 3,name: '入职申请中'},{code: 4, name: '待上岗确认'}];
    },
    computed: {
        tableList(){
