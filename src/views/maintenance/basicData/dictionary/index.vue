  ...
  <div class="app-container calendar-list-container" style="height:calc(100% - 30px)">
    <basic-container style="height:100%;width:100%">
      <el-row style="height: 100%;width: 100%">
        <!--字典分类树-->
        <el-col :span="6" class="zTree_custom">
        ...
        <!-------------------字典列表------------------->
        <el-col :span="8" class="detail_center" v-loading="dictListLoading">
          <div class="item_title">
          ...
          >
            ...
            <el-table-column align="center" label="字典标识" prop="code" show-overflow-tooltip>
        ...
        <!---------------------- 字典对照 -------------------------->
        <el-col :span="9" class="detail_right" v-loading="dictDetailLoading">
          <div class="item_title">
          ...
          >
            <el-table-column align="center" type="index" width="50" label="序号"></el-table-column>
            <el-table-column align="center" label="字典对照码" prop="code" show-overflow-tooltip>
              <template slot-scope="scope">
                <span>{{ scope.row ? scope.row.code : "" }}</span>
              </template>
            </el-table-column>
            <el-table-column align="center" label="字典对照值" prop="name" show-overflow-tooltip>
              <template slot-scope="scope">
                <span>{{ scope.row ? scope.row.name : "" }}</span>
              </template>
            </el-table-column>
            <el-table-column align="center" label="当前状态" prop="dictFlag">
              <template slot-scope="scope">{{ scope.row.dictFlag | statusFilter }}</template>
            </el-table-column>
          </el-table>
        </el-col>
      </el-row>
      ...
      <div class="dialog_main">
        ...
        >
          <el-form-item
            ...
            size="small"
            prop="code"
            v-if="this.dictTypeSelectedNode.id == 29"
          >
            <el-input
              v-model="dictListForm.code"
              :disabled="true"
              v-if="this.dialogListName == '修改字典'"
            ></el-input>
            <el-input v-model="dictListForm.code" v-else></el-input>
          </el-form-item>
          <el-form-item label="字典标识" size="small" prop="code" v-else>
          ...
          <el-form-item label="是否特殊">
            <el-radio-group v-model="dictListForm.isSpecial">
      ...
      <div class="dialog_main">
        ...
        >
          ...
          <!-- <el-form-item label="当前状态" size="small" prop="dictFlag">
            <el-switch v-model="dictDetailForm.dictFlag" active-value="1" inactive-value="0"></el-switch>
          </el-form-item>-->
...
import {
  ...
  delDict,
  putDict
} from "@/api/maintenance/dictionary";
...
export default {
  ...
  data() {
    return {
      ...
      dictDetailRules: {
        ...
        name: [{ required: true, message: "请输入字典对照值", trigger: "blur" }]
      },
  ...
  methods: {
    dictTypeTree() {
      //查询字典类型树
      dictTypeTree({ type: 1 }).then(response => {
        console.log("一级字典数据", response);
        this.dictTypeData = [
          {
            id: "",
            parentId: "",
    ...
    getDictIndexData() {
      ...
      this.dictDetailData = [];
      dictTypeTree({ typeId: this.dictTypeSelectId, type: 2 }).then(
        response => {
          this.dictIndexData = response.data.data;
    ...
    getDictData() {
      ...
      dictTypeTree({ type: 3, listId: this.dictIndexSelectId }).then(
        response => {
          this.dictDetailData = response.data.data;
          //关闭遮罩
    ...
    dictTypeDel() {
      this.$confirm(
        ...
        delDictType({ type: 1, id: this.dictTypeSelectId }).then(res => {
          if (res.data.code == 0) {
            this.$notify({
              title: "成功",
              message: "删除成功",
    ...
    dictTypeSubmit() {
      this.$refs["dictTypeForm"].validate(valid => {
        if (valid) {
          let data = Object.assign({}, this.dictTypeForm, {
            type: 1
          });
    ...
    dictListEdit() {
      ...
      this.dialogListVisible = true;
      let data = this.dictIndexData.filter(item => {
        return item.id == this.dictIndexSelectId;
      })[0];
      Object.keys(this.dictListForm).forEach(item => {
        if (item == "isSpecial") {
          this.dictListForm[item] = parseFloat(data[item]);
          return;
        }
        this.dictListForm[item] = data[item];
      });
    },
    dictListDel() {
      this.$confirm(
        "此操作将永久删除该字典(字典名:" +
          this.dictIndexSelectName +
          "), 是否继续?",
        "提示",
        {
          confirmButtonText: "确定",
        ...
        delDictType({ id: this.dictIndexSelectId, type: 2 }).then(res => {
          if (res.data.code == 0) {
            this.$notify({
              title: "成功",
              message: "删除成功",
    ...
    dictDetailEdit() {
      ...
      // this.dictDetailForm = "";
      this.dictFilterDetail.forEach(item => {
        // console.log(item);
        // console.log(this.dictDetailSelectedRow);
        if (item.id == this.dictDetailSelectedRow.id) {
          this.dictDetailForm = item;
          Object.keys(this.dictDetailForm).forEach(items => {
            this.dictDetailForm[items] = item[items];
          });
        }
      });
    ...
    dictDetailDel() {
      this.$confirm(
        "此操作将永久删除该字典对照(字典对照值:" +
          this.dictDetailSelectedRow.name +
          "), 是否继续?",
        "提示",
        {
          confirmButtonText: "确定",
        ...
        delDict({ id: this.dictDetailSelectedRow.id, type: 3 }).then(res => {
          if (res.data.code == 0) {
            this.$notify({
              title: "成功",
              message: "删除成功",
        ...
        if (valid) {
          if (this.dialogDetailName === "新增字典对照") {
            ...
            putDict({
              ...this.dictDetailForm,
              type: 3,
              id: this.dictDetailSelectedRow.id
            }).then(res => {
              if (res.data.code == 0) {
                this.$notify({
                  title: "成功",
